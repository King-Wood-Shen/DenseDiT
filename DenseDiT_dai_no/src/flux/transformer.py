...
def tranformer_forward(
    ...
    **params: dict,
    ...
    use_context = context_latents is not None
    ...
    context_latents = self.x_embedder(context_latents) if use_context else None
    ...
    if use_context:
        # context_ids[:, :1] = condition_type_ids
        cont_rotary_emb = self.pos_embed(context_ids)
        ...
        if self.training and self.gradient_checkpointing:
            ...
            encoder_hidden_states, hidden_states, condition_latents, context_latents = (
                torch.utils.checkpoint.checkpoint(
                    ...
                    context_latents=context_latents if use_context else None,
                    temb=temb,
                    cond_temb=cond_temb if use_condition else None,
                    cond_rotary_emb=cond_rotary_emb if use_condition else None,
                    cont_temb=cont_temb if use_context else None,
                    cont_rotary_emb=cont_rotary_emb if use_context else None,
            ...
            encoder_hidden_states, hidden_states, condition_latents, context_latents = block_forward(
                block,
                model_config=model_config,
                hidden_states=hidden_states,
                encoder_hidden_states=encoder_hidden_states,
    ...
    for index_block, block in enumerate(self.single_transformer_blocks):
                ...
                **(
                    {
                        ...
                        "cont_temb": cont_temb,
                        "cont_rotary_emb": cont_rotary_emb,
                    }
        ...
        #TODO: check if we need to separate the condition_latents and the context_latents
        if use_condition:
            hidden_states, condition_latents, context_latents = result
        else:
